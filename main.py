...
def main(root_res: resource.Site):
    ...
    while (ip:= ot_mgr.dequeue_child_ip()) is not None:
        root_res.add_resource((ot_mgr.get_child_ips()[ip].uri,), RadarResource(ot_mgr.get_child_ips()[ip].uri,  ot_mgr))
    ...
    asyncio.get_event_loop().run_until_complete(asyncio.gather(main_task(ot_mgr, root_res), ot_mgr.inform_children(), influx_sender.influx_task(ot_mgr)))
...
async def main_task(ot_manager: OtManager, root_res: resource.Site):
    while True:
        ...
        if ip is not None:
            try:
                root_res.add_resource((ot_manager.get_child_ips()[ip].uri,), RadarResource(ot_manager.get_child_ips()[ip].uri, ot_manager))
